...
struct SubBag
{
    ...
    constexpr SubBag(const SubBag& other)
        : name(other.name)
        , index(other.index)
        , count(other.count)
    {}

    constexpr SubBag(SubBag&& other)
        : name(std::move(other.name))
        , index(other.index)
        , count(other.count)
    {}

    constexpr SubBag& operator=(const SubBag& other)
    {
        name = other.name;
        index = other.index;
        count = other.count;
    ...
    constexpr SubBag& operator=(SubBag&& other)
    {
        name = std::move(other.name);
        index = other.index;
        count = other.count;
...
template<size_t MaxNrOfSubs>
struct Bag
{
    StringPair name;
    cxpr::vector<SubBag, MaxNrOfSubs> subs;
};

template<size_t MaxNrOfSubs, size_t MaxNrOfWords>
constexpr void parseBagInto(Bag<MaxNrOfSubs>& out, const cxpr::vector<std::string_view, MaxNrOfWords>& words)
{
    out.name = std::make_pair(words[0], words[1]);
    if (words[4] != "no")
    {
        for (size_t i = 1; i < words.size() / 4; i++)
        {
            out.subs.push_back({});
            out.subs[i-1].name = std::make_pair(words[i * 4 + 1], words[i * 4 + 2]);
            out.subs[i-1].count = cxpr::parse<int>(words[i * 4]);
        }
    }
}

template<size_t NrOfLines, size_t MaxNrOfWordsPerLine>
constexpr auto parseBags(const std::array<cxpr::vector<std::string_view, MaxNrOfWordsPerLine>, NrOfLines>& input)
{
    std::array<Bag<(MaxNrOfWordsPerLine / 4)>, NrOfLines> out = {};
    for (size_t i = 0; i < input.size(); i++)
    {
        parseBagInto(out[i], input[i]);
    }

    for (size_t i = 0; i < out.size(); i++)
    {
        for (size_t j = 0; j < out[i].subs.size(); j++)
        {
            for (size_t k = 0; k < out.size(); k++)
            {
                if (out[i].subs[j].name == out[k].name)
                {
                    out[i].subs[j].index = k;
                    break;
                }
            }
        }
...
template<size_t NrOfBags, size_t MaxNrOfSubs>
constexpr int calculateA(const std::array<Bag<MaxNrOfSubs>, NrOfBags>& input)
{
    bool canReach[NrOfBags] = { false };
    for (size_t i = 0; i < input.size(); i++)
    {
        if (input[i].name.first == "shiny" && input[i].name.second == "gold")
            canReach[i] = true;
    }
    for (size_t i = 0; i < input.size(); i++)
    {
        ...
        {
            for (size_t k = 0; k < input[j].subs.size(); j++)
            {
                if (canReach[input[j].subs[k].index])
                    canReach[j] = true;
            }
        }
    }
...
int main()
{
    constexpr std::string_view input = R"RAW(light red bags contain 1 bright white bag, 2 muted yellow bags.
...
dotted black bags contain no other bags.)RAW";

    constexpr std::string_view input2 = R"RAW(vibrant aqua bags contain 1 shiny magenta bag, 2 muted teal bags, 1 dim magenta bag, 1 muted chartreuse bag.
...
)RAW" R"RAW(clear lime bags contain 1 bright gold bag.
...
)RAW" R"RAW(dim violet bags contain 3 dark brown bags, 4 muted plum bags.
...
dark lavender bags contain 2 striped blue bags, 5 posh blue bags, 2 plaid green bags.)RAW";

    constexpr auto lines = cxpr::split_lines<cxpr::count_lines(input)>(input);
    constexpr auto maxWords = cxpr::count_max_words_per_string(lines);
    constexpr auto words = cxpr::split_words_per_string<maxWords>(lines);
    constexpr auto bags = parseBags(words);

    //for (size_t i = 0; i < bags.size(); i++)
    //{
    //    std::cout << bags[i].name.first << " " << bags[i].name.second << " (" << i << ")" << std::endl;
    //    for (size_t j = 0; j < bags[i].subs.size(); j++)
    //    {
    //        std::cout << "    " << bags[i].subs[j].name.first << " " << bags[i].subs[j].name.second << " (" << bags[i].subs[j].count << "x " << bags[i].subs[j].index << ")" << std::endl;
    //    }
    //}

    constexpr auto a = calculateA(bags);
    std::cout << a << std::endl;
