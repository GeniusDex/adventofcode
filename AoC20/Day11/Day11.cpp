...
constexpr bool isChair(int value)
{
    return (value & 0x10) != 0;
}

constexpr bool isOccupied(int value)
{
    return (value & 0x20) != 0;
}

constexpr int nrNeighbours(int value)
{
    return (value & 0xF);
}

constexpr int withOccupied(int value, bool occupied)
{
    if (occupied)
        return value | 0x20;
    else
...
constexpr size_t findFirstNewline(const std::string_view& input)
{
...
template<size_t NrRows, size_t NrColumns>
constexpr auto parseSeatLayout(const std::string_view& input)
{
    std::array<std::array<int, NrColumns+2>, NrRows+2> out = {};
    size_t y = 1;
    size_t x = 1;
    for (size_t i = 0; i < input.length(); i++)
    {
...
template<size_t NrRows, size_t NrColumns>
void printGrid(const std::array<std::array<int, NrColumns>, NrRows>& grid)
{
    for (size_t y = 1; y < NrRows - 1; y++)
    {
        for (size_t x = 1; x < NrColumns - 1; x++)
        {
...
template<size_t NrRows, size_t NrColumns>
auto calculateA(const std::array<std::array<int, NrColumns>, NrRows>& input)
{
    auto current = input;
    auto next = input;
    do
    {
        for (size_t y = 0; y < NrRows; y++)
        {
            for (size_t x = 0; x < NrColumns; x++)
            {
                next[y][x] = current[y][x] & 0x30;
            }
        }
        ...
        for (size_t y = 1; y < NrRows - 1; y++)
        {
            for (size_t x = 1; x < NrColumns - 1; x++)
            {
                if (!isChair(current[y][x]))
                    continue;

                bool wasOccupied = isOccupied(current[y][x]);
                bool isOccupied = wasOccupied;
                if (nrNeighbours(current[y][x]) == 0)
                    isOccupied = true;
                if (nrNeighbours(current[y][x]) >= 4)
                    isOccupied = false;
                if (isOccupied)
                {
                ...
                next[y][x] = withOccupied(next[y][x], isOccupied);
            }
        }
        cxpr::swap(current, next);
    } while (current != next);
...
//template<size_t NrSeats>
//constexpr auto getAdjacentSeats(const std::string_view& input)
//{
//    std::array<cxpr::vector<int, 8>, NrSeats> out = {};
//    size_t y = 0;
//    size_t x = 0;
//    size_t index = 0;
//    std::string_view lastLine = input;
...
//                if ((x > 0) && (input[i - 1] == 'L')
//                {
//                    input[index].push_back(index - 1);
//                    input[index - 1].push_back(index);
//                }
//                index++;
//            }
//
...
int main()
{
    constexpr auto input = R"RAW(L.LL.LL.LL
LLLLLLL.LL
L.L.L..L..
LLLL.LL.LL
L.LL.LL.LL
...
L.LLLLL.LL)RAW";
    ...
    constexpr auto input2 = R"RAW(LLLLL.LLLL.LL..LLLLLLLLLLLLLLLLLLLLLLLLL.LLLLLLLLLLLLLLL.L.LLLLLLLLL.LLLLLLL.LLLL.LLLLLLLL
...
LLLLLLLLLLLLLL.LL.LLLLLL.LLLL.L..LLLLLLL.LLL.LL.LLLLLLLLLL.LLLLLLLLL.L.LLLLL.LLLL.LLLLLL.L)RAW";

    constexpr auto nrRows = cxpr::count_lines(input);
    constexpr auto nrColumns = findFirstNewline(input);
    constexpr auto grid = parseSeatLayout<nrRows, nrColumns>(input);

    //constexpr auto nrSeats = cxpr::count_char(input, 'L');
    //constexpr auto pairs = getSeatPairs<nrSeats * 8>(input);

    printGrid(grid);

    const auto a = calculateA(grid);
    std::cout << a << std::endl;
