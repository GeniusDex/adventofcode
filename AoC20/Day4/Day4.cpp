...
constexpr int parseInt(const std::string_view& in)
{
    int out = 0;
...
constexpr bool isValidBYR(const std::string_view& value)
{
    auto year = parseInt(value);
    return (1920 <= year) && (year <= 2002);
}

constexpr bool isValidIYR(const std::string_view& value)
{
    auto year = parseInt(value);
    return (2010 <= year) && (year <= 2020);
}

constexpr bool isValidEYR(const std::string_view& value)
{
    auto year = parseInt(value);
    return (2020 <= year) && (year <= 2030);
}

constexpr bool isValidHGT(const std::string_view& value)
{
    auto quantity = parseInt(value.substr(0, value.length() - 2));
    auto unit = value.substr(value.length() - 2);
    if (unit == "cm")
        return (150 <= quantity) && (quantity <= 193);
    else if (unit == "in")
        return (59 <= quantity) && (quantity <= 76);
    else
        return false;
}

constexpr bool isValidHCL(const std::string_view& value)
{
...
constexpr bool isValidECL(const std::string_view& value)
{
    return (value == "amb")
        || (value == "blu")
        || (value == "brn")
...
constexpr bool isValidPID(const std::string_view& value)
{
    return (value.length() == 9) && (parseInt(value) >= 0);
}

constexpr bool isValidA(const std::string_view& str)
{
    int nrPresent = 0;
    constexpr std::array<std::string_view, 7> keys = { "byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid" };
...
constexpr bool isValidB(const std::string_view& str)
{
    int nrValid = 0;
    constexpr std::array<std::string_view, 7> keys = { "byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid" };
    constexpr bool (*(isValidFns[7]))(const std::string_view&) = { isValidBYR, isValidIYR, isValidEYR, isValidHGT, isValidHCL, isValidECL, isValidPID };
    ...
    for (size_t i = 0; i < str.length(); i++)
    {
        ...
        {
            auto value = str.substr(start, i - start);
            if (keyId < 1000 && isValidFns[keyId](value))
                nrValid++;
            start = i + 1;
            keyId = -1;
        }
...
constexpr int calculate(const std::string_view& input, FnType isValidFn)
{
    int nrValid = 0;
    ...
    for (size_t i = 1; i < input.length(); i++)
    {
        if (input[i - 1] == '\n' && input[i] == '\n')
        {
            if (isValidFn(input.substr(start, i - start)))
                nrValid++;
            start = i + 1;
        }
    }

    if (isValidFn(input.substr(start)))
        nrValid++;
...
int main()
{
    constexpr std::string_view input1 = R"RAW(ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
iyr:2011 ecl:brn hgt:59in)RAW";

    constexpr std::string_view input = R"RAW(eyr:2028 iyr:2016 byr:1995 ecl:oth
...
pid:#725759
hcl:#602927 iyr:2013 byr:2003 eyr:2023 cid:100)RAW";

    constexpr auto a = calculate(input, isValidA);
    constexpr auto b = calculate(input, isValidB);
    std::cout << a << std::endl;
    std::cout << b << std::endl;
