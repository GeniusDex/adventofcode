...
constexpr int countAnsweredByAnyone(const std::string_view& input)
{
    bool answered[26] = { false };
    for (size_t i = 0; i < input.length(); i++)
    {
        if (input[i] >= 'a' && input[i] <= 'z')
            answered[input[i] - 'a'] = true;
    }
    int answers = 0;
    for (size_t i = 0; i < 26; i++)
        if (answered[i]) answers++;
    return answers;
}

constexpr void findMissing(const std::string_view& input, bool(&missing)[26])
{
    for (size_t i = 0; i < 26; i++)
        missing[i] = true;
    ...
    {
        if (input[i] >= 'a' && input[i] <= 'z')
            missing[input[i] - 'a'] = false;
    }
}

constexpr void applyMissing(bool(&missing)[26], bool(&answered)[26])
{
    for (size_t i = 0; i < 26; i++)
        if (missing[i])
            answered[i] = false;
...
constexpr int countAnsweredByEveryone(const std::string_view& input)
{
    bool answered[26] = { true };
    for (size_t i = 0; i < 26; i++)
        answered[i] = true;

    bool missing[26] = { false };
    size_t start = 0;
    for (size_t i = 0; i < input.length(); i++)
    {
        if (input[i] == '\n')
        {
            findMissing(input.substr(start, i - start), missing);
            applyMissing(missing, answered);
            start = i + 1;
        }
    }
    findMissing(input.substr(start), missing);
    applyMissing(missing, answered);
...
constexpr int sumGroups(const std::string_view& input, CountFn countFn)
{
    int result = 0;
    ...
    for (size_t i = 1; i < input.length(); i++)
    {
        if (input[i - 1] == '\n' && input[i] == '\n')
        {
            result += countFn(input.substr(start, i - start - 1));
            start = i + 1;
        }
    }
    result += countFn(input.substr(start));

    return result;
}

constexpr int calculateA(const std::string_view& input)
{
    return sumGroups(input, countAnsweredByAnyone);
}

constexpr int calculateB(const std::string_view& input)
{
    return sumGroups(input, countAnsweredByEveryone);
}

int main()
{
    constexpr auto input1 = R"RAW(abc
...
b)RAW"sv;
    ...
    constexpr auto input = R"RAW(zvxc
dv
vh
xv
jvem
...
gdrakz)RAW"sv;
    ...
    constexpr auto a = calculateA(input);
    constexpr auto b = calculateB(input);
